...
import "./JJ_PoundsTeamMngt.sol";

contract JJ_PoundsCrowdSale  is ReentrancyGuard,JJ_PoundsTeamManagement{
    /**
    ...
    */
    ...
    constructor(
                JJ_PoundsToken token,
                JJ_PoundsInvestorsVault vault,
                IERC20 _BUSD,
    ...
    ReentrancyGuard()
    JJ_PoundsTeamManagement(DevMarketing,TeamToken,ListingLiquidity,OperationsManagement,token,vault)
    { 
    ...
    function participateBNB(uint80 _roundId) payable public onlyWhileOpen returns (bool){
        ...
        emit BuyJJP(msg.sender,_numberOfTokens,msg.value); 
        return true;
    }
    function participateBUSD(uint80 _roundId) public onlyWhileOpen returns(bool){
        ...
        uint256 _numberOfTokens = _JJ_PoundsReceiving(bnbEquv,_roundId);
        ...
        emit BuyJJP(msg.sender,_numberOfTokens,busdVal); 
       return true;
    }
    function _JJ_PoundsReceiving(uint256 _weiSent, uint80 _roundId) internal view returns (uint256 ){
        int _channelRate = 0;
        _channelRate  =  getBNBUSDPrice(_roundId);
        int _JJPRate = int(_rate)/(_channelRate/int(_crossDecimal));
    ...
    */
    ...
    function getSoftCap() public view returns (uint256){
        return JJ_PoundsSoftCap;
    }
    function getHardCap() public view returns (uint256){
        return JJ_PoundsHardCap;
    }

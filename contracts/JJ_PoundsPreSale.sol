...
import "./JJ_PoundsToken.sol"; 
import "@openzeppelin/contracts/access/Ownable.sol";
import "./JJ_PoundsInvestorsVault.sol"; 
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@chainlink/contracts/src/v0.6/interfaces/AggregatorV3Interface.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract JJ_PoundsPreSale is Ownable,ReentrancyGuard{
    using SafeMath for uint256;
    JJ_PoundsToken _token;
    JJ_PoundsInvestorsVault  _vaultContract;
    ...
    constructor(JJ_PoundsToken token, address payable vcsbucket, JJ_PoundsInvestorsVault vault) Ownable()ReentrancyGuard() {
        _token = token;
    ...
    function approveWinnerForCollection(uint _type,uint80 _roundId) public nonReentrant() returns(bool){
        ...
        _token.transfer(msg.sender, JJPToken);
        amaWinners[msg.sender] = 0;
        return true;
    }
    function getCollection(uint _type,uint80 _roundId) public view returns(uint256 busdSent,uint bnbVal,uint256 JJPToken){
        ...
        uint256 numberOfTokens = _JJ_PoundsReceiving(bnbEquv,bnb);
        return (busdVal, bnbEquv, numberOfTokens);
    }
    function _JJ_PoundsReceiving(uint256 _weiSent,int bnb) internal view returns (uint256 ){
        int _JJPRate = int(_rate)/(bnb/int(_crossDecimal));
        uint256 _weiJJPRate =  uint256((_JJPRate * 10 **18 )/int(_crossDecimal));
        uint256 tempR = _weiSent.div(_weiJJPRate);
        return tempR * 10 ** 18;
    }
